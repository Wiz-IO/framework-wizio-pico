...
#if 0
static void lfs_print_config(lfs_context_t *c)
{
    LFS_PRINTF("    [CTX] lfs_context_t     = %p\n", c);
    LFS_PRINTF("    [CTX] OP                = %p\n", c->op);
    LFS_PRINTF("    [CTX]   .init           = %p\n", c->op->init);
    LFS_PRINTF("    [CTX]   .mount          = %p\n", c->op->mount);
    LFS_PRINTF("    [CTX] CFG               = %p\n", c->cfg);
    LFS_PRINTF("    [CTX]   .read_size      = %d\n", (int)c->cfg->read_size);
    LFS_PRINTF("    [CTX]   .prog_size      = %d\n", (int)c->cfg->prog_size);
    LFS_PRINTF("    [CTX]   .cache_size     = %d\n", (int)c->cfg->cache_size);
}
#endif
...
#warning LFS_ROM_PRE_FORMAT
    ...
    if ((err = lfs_toerror(lfs_mount(LFS_FS_LFS, LFS_FS_CFG))))
        if (0 == (err = lfs_toerror(lfs_format(LFS_FS_LFS, LFS_FS_CFG))))
            err = lfs_toerror(lfs_mount(LFS_FS_LFS, LFS_FS_CFG));
